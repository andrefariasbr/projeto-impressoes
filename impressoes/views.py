...
def editar_pedido(request, pedido_id):
    ...
    return render(request, 'impressoes/editar_pedido.html', {'form': form, 'pedido': pedido})
...
def detalhar_pedido_professor(request, pedido_id):
    pedido = get_object_or_404(PedidoDeImpressao, id=pedido_id, usuario=request.user)
    return render(request, 'impressoes/detalhar_pedido_professor.html', {'pedido': pedido})
...
def painel_admin(request):
    ...
    if request.method == 'POST':
        ...
        if acao == 'aprovar':
            ...
            messages.success(request, 'Pedido excluído!')
        return redirect('painel_admin')
    return render(request, 'impressoes/painel_admin.html', {
        'pedidos': page_obj,
        'page_obj': page_obj,
        'status': status,
        'busca': busca,
...
def painel_professor(request):
    ...
    total_pendentes = PedidoDeImpressao.objects.filter(usuario=request.user, status='pendente').count()
    total_entregues = PedidoDeImpressao.objects.filter(usuario=request.user, status='concluido').count()
    ...
    if request.method == 'POST':
        ...
        if acao == 'excluir':
            ...
            messages.success(request, 'Pedido excluído!')
        return redirect('painel_professor')
    return render(request, 'impressoes/painel_professor.html', {
        'pedidos': page_obj,
        'page_obj': page_obj,
        'status': status,
        'busca': busca,
